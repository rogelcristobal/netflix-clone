      ...
      >
        ...
        <div className="overflow-y-scroll overflow-x-hidden h-full w-full px-0   pb-72 box-border space-y-2 scroll-smooth	">
          <div className="lg:h-[80vh]  w-full relative box-border  overflow-hidden">
            ...
            <Box className=" absolute top-60 left-16 box-border">
              {/* titles */}
              <h5 className=" text-white text-5xl   font-bold box-border  m-0 mb-6">
                {data?.title}
                {data?.name}
              </h5>
                ...
                {!isLoading
                  ? genres.slice(0,3).map((item, id) => (
                      <GenreChip
                        ...
                        path={pathname}
                        key={id}
                        sx="bg-gray-600/30 hover:bg-gray-400/30 py-1"
                      ></GenreChip>
                    ))
                  : null}
              </Stack>
